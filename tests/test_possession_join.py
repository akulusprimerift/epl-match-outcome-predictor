"""Tests for the Phase 6 resumable possession collection and exact join."""
...
class RepositoryPossessionReportTests(unittest.TestCase):
    """Verify committed Phase 6 reports match the current canonical table."""

    def test_repository_coverage_report_is_reproducible(self) -> None:
        canonical = pd.read_csv(
            PROJECT_ROOT / "data/processed/canonical_matches.csv",
            dtype={"season": "string"},
        )
        expected = build_possession_coverage(canonical, 0.95).frame
        actual = pd.read_csv(
            PROJECT_ROOT / "reports/possession_coverage.csv",
            dtype={
                "season": "string",
                "team": "string",
                "model_b_period_start": "string",
            },
            keep_default_na=False,
        )
        pd.testing.assert_frame_equal(actual, expected, check_dtype=False)
        self.assertEqual(set(actual["scope"]), {"season", "team"})

    def test_join_exception_reports_have_explicit_schemas(self) -> None:
        for name in ("possession_unmatched.csv", "possession_ambiguous.csv"):
            with self.subTest(report=name):
                report = pd.read_csv(PROJECT_ROOT / "reports" / name)
                self.assertEqual(
                    tuple(report.columns), POSSESSION_JOIN_REPORT_COLUMNS
                )
